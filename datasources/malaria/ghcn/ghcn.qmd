...
In mechanistic models, like the SEACR-SEI model, these daily observations are fed directly into the system's differential equations. Temperature (T) is used to dynamically calculate the mosquito biting rate, daily oviposition (egg-laying) rate, mosquito mortality, and the parasite's Extrinsic Incubation Period (EIP) using non-linear thermal performance curves. Precipitation (P) can act as a proxy for the availability of aquatic breeding habitats, driving the environmental carrying capacity (K). By accurately capturing phenomena like "larval flushing" during heavy storms or thermal suppression during winter months, modelers can simulate highly localised transmission intensity, estimate shifts in the basic reproduction number (R_0), and project future transmission scenarios under varying climate conditions.
...
ggplot() +
  ...
  labs(
    title = ("Daily Average Temperature values in Limpopo and Maputo \nfrom 2010 to 2025"),
    x = "Year",
...
The environmental drivers yield distinct biological outcomes for the two regions. In Limpopo, mosquito density follows an opportunistic "boom-and-bust" cycle, spiking dramatically in response to summer rainfall and crashing during dry periods. Maputo’s more frequent, moderate rainfall may provide a much more stable carrying capacity for aquatic stages. This allows Maputo to maintain sustained mosquito population levels even through traditionally drier periods ensuring year-round vector presence and biting pressure.
...
df |> 
  ...
  labs(
    ...
    x = "Year",
    y = "Number of cases",
    caption = str_wrap("Biological limits of transmission stability. Maputo maintains a perennial symptomatic burden because environmental conditions rarely fall below the threshold for parasite development. In contrast, Limpopo’s transmission is strictly seasonal; case numbers drop to near zero during winter thermal troughs, indicating that cold-stress on the parasite and vector acts as a natural annual (break) in the transmission chain. Source: SEACR-SEI Model.")
  ) +
  ylim(0, NA)
```
...
In areas where environmental conditions support year-round vector competence, policy may focus on sustained, continuous interventions. This includes ensuring universal coverage of Long-Lasting Insecticidal Nets (LLINs), robust seasonal malaria chemoprevention (SMC), and continuous supply chains for rapid diagnostic tests (RDTs) and antimalarials.

The juxtaposition of an endemic zone (Maputo) and an epidemic-prone zone (Limpopo) highlights the need for regional data sharing. Asymptomatic human carriers moving across borders can act as an infectious reservoir, igniting outbreaks in neighboring regions when local weather conditions become highly permissive.
